...
tcentroids -> stores centroids of previous iteration
*/
int *cluster_count;
int *cluster_map;
int *pre_cluster_map;
...
void init()
{
	...
	tcentroids = (float **)calloc(clusters,sizeof(float*));
    for(i=0;i<clusters;i++)
	{
        centroids[i] = (float *)calloc(total_features+1,sizeof(float));
		tcentroids[i] = (float *)calloc(total_features+1,sizeof(float));
	}
...
float traditional_kmeans(int run)
{
	...
	for(k=0;k<MAX_ITERATIONS;k++)
	{
		...
		for(i=0;i<rowind;i++)
		{
			...
			for(j=rowptr[i];j<rowptr[i+1];j++) 
			{
				//update the centroid. Note:- here we use tcentroids for doing batch updates and value is later copied to centroids. Necessary for batch updates
